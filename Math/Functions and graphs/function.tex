...
\begin{document}
    ...
    Let us consider the area of a triangle first. Suppose we are given $a,b,c$ as three sides of a triangle and $\alpha,\beta,\gamma$ be the angle opposite to $a,b,c$ respectively.

    Let us consider the area of succh triangle with $b$ as its base first. We have the following two situations: if $\gamma>90^\circ$ or $\gamma<90^\circ$.

    If $\gamma<90^\circ$, we see that the height of the triangle is indeed equal to $a\sin{\gamma}$, so the area is $\frac{1}{2}ab\sin{\gamma}$; for if $\gamma>90^\circ$, the height is $a\sin(180^\circ - \gamma)$, so the area becomes $\frac{1}{2}ab\sin(180^\circ - \gamma)$. One interesting result is that when we set up the relation 
