...
\pagestyle{fancy}
\fancyhf{}
\fancyhf[HL]{A short course on Functions}
...
\begin{document}
    ...
    Every function has its graph by identifying all the points $(x,y)$ that satisfies the equation $y=f(x)$. The set $G=\{(x,y):y=f(x)\}$ is then of course suitable for the discussion on quadratic equation.

    \subsubsection*{Obtaining vertex form using Transformation}

    Let's start with the simplest quadratic graph. We have $y=x^2$ as follows.
    ...
    From the information in the section of transformation of graphs of function, we may now move the whole graph upward by $k$ units and rightward by $h$ units. It is the same as transforming \[f(x)\mapsto f(x-h)+k\] and therefore we can conclude the graph opening upward with vertex $(h,k)$ is given by the relation $y=(x-h)^2+k$, drawn as follows.
    ...
    Moreover, it is compatible with reflection and dilation. For if every $y$-coordinates of the graph is reflected along $x$-axis, the whole graph becomes opening downward. It means every function $f(x)=a(x-h)^2+k$ with $a<0$ is opening downward, and opening upward with $a>0$.

    We now conclude this form of quadratic equation to be the \textbf{vertex form} of quadratic equation \[f(x)=a(x-h)^2+k\] with $a\neq 0$, $h,k\in\mathbb{R}$.
