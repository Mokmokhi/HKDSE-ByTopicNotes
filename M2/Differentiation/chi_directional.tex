...
\begin{document}
    ...
    外積的定義可考慮面積與體積的計算原理：考慮三個單位向量$\hat{i},\hat{j},\hat{k}$的乘積為體積及任意兩個向量的乘積為面積，基於$\hat{i}\times \hat{j}$為$i-j$平面的面積單位，而面積乘以高等於體積，故$V(\hat{i},\hat{j},\hat{k})=(\hat{i}\times \hat{j})\cdot \hat{k}$為標量，使得$\hat{i}\times\hat{j}=\hat{k}$。
