...
\begin{document}
    ...
    Our first step to complex binomial is the agreement of complex power, so we shall consider what does it mean by $x^i$. It could be a complex decomposition of $\cos{\ln{x}}+i\sin{\ln{x}}$ follows from Euler's, but it is meaningless to our construction. We now revisit some tools.

    \begin{proposition}
        Let $z,w\in\mathbb{C}$, then \[\derivative{z^w}{z}=wz^{w-1}.\]
    \end{proposition}
